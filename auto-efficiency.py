...
#using entropy
my_tree = DecisionTree(criterion = 'entropy', max_depth = 5) 
my_tree.fit(X_train, y_train)
my_y_hat = my_tree.predict(X_test)
my_rmse = rmse(my_y_hat, y_test)
print(f"My Decision Tree(criterion = entropy)'s RMSE = {my_rmse:.2f}")

#using gini_index
my_tree = DecisionTree(criterion = 'gini_index', max_depth = 5) 
my_tree.fit(X_train, y_train)
my_y_hat = my_tree.predict(X_test)
my_rmse = rmse(my_y_hat, y_test)
